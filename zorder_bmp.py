...
def Zorder(df, cols, original_df, name):
    ...
    original_df.to_csv(f"./datasets/{name}-bmp-zorder.csv", index=False)
...
def ZorderBlock(df, cols, block_size):
    ...
    if not os.path.exists(save_path):
        import json
        scan_conds = json.load(open('./datasets/scan_conds.json', 'r'))
        print('OKOK')
        # assert 0
        # Queries, scan_conds_1 = QueryGeneration(100, df.loc[:, cols], cols)
...
if __name__ == "__main__":
    ...
    # all_cols = cols
    cols = ['0', '1', '2']
    all_cols = list(map(str, range(10)))
    ...
    table = datasets.LoadRandomWalkBMTree(colsNum, rowNum)
